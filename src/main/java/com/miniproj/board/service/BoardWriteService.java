...
public class BoardWriteService implements BoardService {
	...
	private BoardVo getData(HttpServletRequest req) throws FileUploadException, UnsupportedEncodingException, NamingException, SQLException {
		String upload = "\\board\\imgs";

		// 현재 request 대응하는 서블릿 객체를 얻음
		ServletContext context = req.getServletContext();

		// 파일이 업로드될 물리적 경로
		String realPath = context.getRealPath(upload);
		System.out.println("파일이 저장될 새로운 경로 : " + realPath);
...
//					writer = loginMember.getUserId();
				...
				imgFile = FileNameProcessing.getNewFileName(fi, realPath); // 중복되지 않는 새로운 파일 이름 얻기
				...
				try {
					...
					// 유저가 업로드한 파일이 저장이 안되었다. -> 글 등록은 되어야함.
					imgFile="";
				}
			}
		}
		...
		String dbUserImg = "";
		// DB에 insert 하기전 업로드된 파일이 있는지 확인
		if(!imgFile.equals("")) { // 업로드된 이미지가 존재 한다면
			dbUserImg = "board/imgs/" + imgFile; // DB에 경로까지 포함해서 insert한다.
		}
